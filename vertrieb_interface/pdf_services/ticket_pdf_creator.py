...
def createTicketPdf(data, vertrieb_angebot, user):
    ...
    user_folder = os.path.join(
        settings.MEDIA_ROOT, f"pdf/usersangebots/{user.username}/Tickets/"
    )
    if not os.path.exists(user_folder):
        os.makedirs(user_folder)

    output_file = os.path.join(user_folder, f"Ticket_{vertrieb_angebot.angebot_id}.pdf")

    # create the directory if needed
    if not os.path.exists(user_folder):
        os.makedirs(user_folder)
        outputPath = os.path.join(
            user_folder, f"Ticket_{vertrieb_angebot.angebot_id}.pdf"
        )
